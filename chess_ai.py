...
def find_best_move(game_state, valid_moves, return_queue=None):
    ...
    # Kiểm tra xem có phải trạng thái chiếu hết không
    is_check = game_state.in_check
    is_last_move = len(valid_moves) == 1 and is_check

    # Nếu đây là nước cuối cùng trước khi bị chiếu hết
    if is_last_move:
    ...
    # Nếu không tìm thấy nước đi nào và còn nước đi hợp lệ, chọn ngẫu nhiên từ các nước tốt nhất
    if next_move is None and valid_moves:
        ordered_moves = order_moves(game_state, valid_moves)
        next_move = ordered_moves[0]  # Lấy nước đi tốt nhất theo đánh giá nhanh
